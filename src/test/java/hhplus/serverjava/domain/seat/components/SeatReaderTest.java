...
public class SeatReaderTest {
	...
	void findAvailableSeatsEmptyTest() {
		...
		BaseException exception = assertThrows(BaseException.class,
			() -> seatReader.findAvailableSeats(testId, testDateTime, Seat.State.AVAILABLE));
	...
	void findAvailableSeatTest() {
		...
		when(seatReaderRepository.findAvailableSeat(testId, testDateTime, Seat.State.AVAILABLE, 50)).thenReturn(
			Optional.of(seat));
	...
	void findAvailableSeatNotFoundTest() {
		...
		//when & then
		BaseException exception = assertThrows(BaseException.class,
			() -> seatReader.findAvailableSeat(testId, testDateTime, Seat.State.AVAILABLE, 50));
